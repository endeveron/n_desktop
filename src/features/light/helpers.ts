...
export function shouldRefetch({
  ...
  const STALE_MS = staleMinutes * MINUTE;
  const isStale = Date.now() - lastUpdated > STALE_MS;

  return isStale;
}

export async function nukeAllModals(page: Page) {
  await page.evaluate(() => {
    const selectors = [
      '.modal',
      '.modal.is-open',
...
export function updateLightSchedule(lightData: LightData): LightData {
  const todayIndex = getTodayIndex(lightData.weekSchedule.schedule);
  const tomorrowIndex = getTomorrowIndex(todayIndex);
  const hasTodayData = hasValidTimeIntervalsData(lightData.today);
  const hasTomorrowData = hasValidTimeIntervalsData(lightData.tomorrow);

  const updatedSchedule = lightData.weekSchedule.schedule.map((day, index) => {
    // Update today's schedule if valid data exists
    if (index === todayIndex && hasTodayData) {
      return {
        ...day,
        hours: generateHoursArray(lightData.today),
      };
    }
    ...
    if (index === tomorrowIndex && hasTomorrowData) {
      return {
        ...day,
        hours: generateHoursArray(lightData.tomorrow),
      };
    }
...
export async function logPossibleBlockers(page: Page) {
  try {

...
import getInviteModel from '@/features/auth/models/invite';
import getUserModel from '@/features/auth/models/user';
import {
  CreateUserArgs,
  Credentials,
  JwtEmailPayload,
...
import { EmailType, ServerActionResult } from '@/types/index';
...
export const createInviteCode = async (): Promise<ServerActionResult> => {
  try {
    const InviteModel = await getInviteModel();
...
export const onboardUser = async ({
  ...
  try {
    ...
    if (!user) {
      return handleActionError('Unable to find a user with provided ID');
    }
